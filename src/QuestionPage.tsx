...
export const QuestionPage: FC<RouteComponentProps<RouteParams>> = ({
  ...
  const setUpSignalRConnection = async (questionId: number) => {
    const connection = new HubConnectionBuilder()
      .withUrl('https://localhost:44396/questionshub')
  ...
  useEffect(() => {
    ...
    if (match.params.questionId) {
      let qId = Number(match.params.questionId);
      getQuestion(qId).then((q) => setQuestion(q));

      setUpSignalRConnection(qId).then((con) => {
        connection = con;
      });
    }
